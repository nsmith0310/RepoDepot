...
class DDQN:
    ...
    def _build_model(self):       
        if self.network=='scratch_net':
            net = scratchnet(self.state_size,self.action_size).to(self.device)
        else:
            net = mobilenet(self.state_size,self.action_size).to(self.device)           
        return net
    
    def load_trained_model(self):
        MODEL_DIR = Path(__file__).parent / "model"
        state_dict = torch.load(MODEL_DIR / "main_model.pth",map_location=self.device, weights_only=True)
        self.main_model.load_state_dict(state_dict); None
        self.main_model.to(self.device); None
        self.epsilon = 0
